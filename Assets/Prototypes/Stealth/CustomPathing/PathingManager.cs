...
public class PathingManager : MonoBehaviour
{
    ...
    private void ResetCells()
    {
        for (int x = BottomCorner.x; x < TopCorner.x; x++)
        {
            for (int y = BottomCorner.y; y < TopCorner.y; y++)
            {
                pathingCells[x][y].g = 0;
                pathingCells[x][y].h = 0;
                pathingCells[x][y].f = 0;
                pathingCells[x][y].fromCell = null;
            }
        }
    }
            ...
            for (int n = 0; n<neighbors.Count-1; n++)
            {
                ...
                if (!closedSet.Contains(neighbor) && !neighbor.blocked)
                {
                    ...
                    if (newPath)
                    {
                        ...
                        neighbor.h = Vector2.Distance(currentCoord, goal); //hueristic is just euclidean distance
